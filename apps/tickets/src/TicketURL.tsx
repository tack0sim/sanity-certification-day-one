...
export function TicketURL(props: DocumentHandle) {
  ...
  return (
    <>
      <Box flex={1}>
        <TextInput type="url" value={value || ''} onChange={(event) => editTicketURL(event.currentTarget.value)} />
      </Box>
      <Button href={value} target="_blank" disabled={!isValid} text="Open" tone="primary" mode="ghost" as="a" />
    </>
  );
}
