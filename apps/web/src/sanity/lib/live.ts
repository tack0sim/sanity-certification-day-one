...
import { defineLive, resolvePerspectiveFromCookies, type SanityFetchOptions } from 'next-sanity/experimental/live'; // experimental next-sanity live with nextjs 16 'use cache'
...
const sanityFetch = async <const QueryString extends string>({
  ...
  const perspective = isDraftMode ? await resolvePerspectiveFromCookies({ cookies: await cookies() }) : 'published';
