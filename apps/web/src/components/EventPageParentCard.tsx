...
export default async function EventPageParentCard({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { name, date, headline, details, format, doorsOpen, venue, tickets } = event;

  const eventDate = new Date(date).toDateString();
  const eventTime = new Date(date).toTimeString();
  const doorsOpenTime = new Date(new Date(date).getTime() - doorsOpen * 60000).toLocaleDateString();

  const imageUrl = headline?.photo
    ? urlFor(headline.photo).height(310).width(550).quality(80).auto('format').url()
        ...
        <div className="grid items-top gap-12 sm:grid-cols-2">
          ...
          <div className="flex flex-col justify-center space-y-4">
            <div className="space-y-4">
              ...
              {name ? (
                <h1 className="text-4xl font-bold tracking-tighter mb-8 text-gray-900 dark:text-white">{name}</h1>
              ) : null}
              {headline?.name ? (
